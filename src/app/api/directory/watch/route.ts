...
export interface DirectoryWatchEvent {
  type: 'connected' | 'add' | 'addDir' | 'change' | 'unlink' | 'unlinkDir' | 'error' | 'heartbeat';
  path?: string;
  relativePath?: string;
  stats?: {
    isFile: boolean;
...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      start(controller) {
        ...
        const heartbeat = setInterval(() => {
          try {
            ...
            console.log('[DirectoryWatch SSE] Client disconnected, cleaning up');
    ...
    console.error('[DirectoryWatch SSE] Error setting up directory watch:', error);
    return NextResponse.json(
      { 
        success: false, 
