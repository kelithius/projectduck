...
interface SecurityLog {
  timestamp: Date;
  type: 'path_traversal' | 'forbidden_extension' | 'file_too_large' | 'malicious_pattern';
  requestPath: string;
  clientInfo: {
    userAgent?: string;
    ip?: string;
...
export class SecurityService {
  ...
  private static readonly ALLOWED_HIDDEN_FILES = [
    'gitignore', 'env', 'editorconfig', 'prettierrc', 'eslintrc', 'nvmrc', 'node-version'
  ];
  ...
  private static logSecurityEvent(log: Omit<SecurityLog, 'timestamp'>): void {
    ...
    const logLevel = log.severity === 'critical' || log.severity === 'high' ? 'error' : 'warn';
  ...
  private static isAllowedHiddenFile(filename: string): boolean {
    ...
    return this.ALLOWED_HIDDEN_FILES.some(allowed => {
      ...
      if (allowed === 'eslintrc' || allowed === 'prettierrc') {
        return nameWithoutDot.match(new RegExp(`^${allowed}\\.(js|json|yml|yaml)$`));
      }
      
      // .env 只允許特定模式 (.env.local, .env.development 等)
      if (allowed === 'env') {
        return nameWithoutDot.match(/^env\.(local|development|staging|production)$/);
      }
   ...
   * 取得客戶端資訊 (用於安全記錄)
   */
  private static getClientInfo(): { userAgent?: string; ip?: string; timestamp?: string } {
    ...
    if (typeof window !== 'undefined') {
      ...
      clientInfo.ip = process.env.REMOTE_ADDR || process.env.HTTP_X_FORWARDED_FOR || 'unknown';
    }
  ...
  static validatePath(requestPath: string = '', basePath: string): string {
    ...
    const normalizedResolved = path.normalize(resolvedPath).replace(/[\/\\]/g, path.sep);
    const normalizedBase = path.normalize(resolvedBasePath).replace(/[\/\\]/g, path.sep).replace(/[\/\\]+$/, '');
